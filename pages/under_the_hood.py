...
st.write("")
st.write("Our application gathers information from property websites like Zoopla.co.uk and Rightmove.co.uk, including the images uploaded for each property. These images are then analysed by our AI model, which is fine tuned to detect specific features such as natural light in rooms, presence of carpets, and more. Each image is rated across different criteria, and these ratings are combined to create an overall score for each property. When you search with specific preferences, the results are ranked based on these scores and the top results are the ones that best match your preferences making searching for your next home faster and easier.")
